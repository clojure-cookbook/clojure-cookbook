...
For example, this HTML file is an Enlive template:
...
The following Clojure code defines an Enlive template based on this
HTML:
...
The result, printed out with newlines, is:
...
- +[:div.sidebar]+ selects all +<div>+ element nodes with a CSS style
  of "sidebar".
