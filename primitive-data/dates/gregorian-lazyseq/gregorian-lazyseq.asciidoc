...
You can use Java's +java.util.GregorianCalendar+ class and methods coupled with
...
dates into a huge variety of output formats.
...
infinite. Consequently, in the examples above, we are always careful to
evaluate our result sequences in contexts (such as such as +take+ and +drop+)
which limit the values produced.
...
unit. For each call of the function, we either return a copy of the Gregorian
calendar object or a formatted string derived from the calendar object. (To
avoid mysterious and unintended side-effects, it is advisable to avoid
returning the mutated object directly).
...
We can generalize the first example to a function which takes a starting year
...
Using the +java.text.SimpleDateFormat+ class we can then format
the dates in a huge variety of different formats:
