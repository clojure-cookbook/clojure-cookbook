...
"multvector" as well, that can store multiple values at each index
(provided they are added using +insert+.)

Reading the code, you'll notice that good deal of the actual logic is
...
inserting and deleting items, requiring to the functions to run a
...
This is a design decision that has a several benefits and
...
eliminate most of the type checking, wrapping and unwrapping forms.
...
There is one more feature of this code that deserves commentary; that
is, the use of +::multi-value+ metadata on the sets used to store
