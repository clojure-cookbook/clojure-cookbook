...
You need a way to perform fast, efficient in-memory searches across a
...
At first, a binary search tree - or a binary search over a list -
might seem like a good solution. However this only works on sorted
data but from the problem statement, you know you'll need something
better. In the case of a binary search tree, it could end up quite
unbalanced and no more efficient than a linked list.

Instead you'll use a Red-Black tree, a type of self-balancing binary
search tree.

Before you get started, make sure you add
https://github.com/clojure/core.match[core.match] as a dependency to
your project:
...
Now, define the +balance+ and +insert-val+ functions which are the
core of our Red Black Tree:
