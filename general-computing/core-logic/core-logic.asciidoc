...
The _core.logic_ library is a Clojure implementation of the
_miniKanren_ Domain Specific Langauge for logic programming. It's
...
_miniKanren_ is a Domain Specific Language written in Scheme, intended
to give many of the benefits of a logic programming language (such as
Prolog) from within Scheme. An implementation of _miniKanren_ was
...
In general, all _core.logic_ queries begin with one of the libraries
...
+clojure.core.logic/fresh+) ground to values or constrained by logic
statements.
...
evaluation. Unification uses substitution of variables for values in
...
many times to find the elements. Using _clojure.core.logic/to-stream_
