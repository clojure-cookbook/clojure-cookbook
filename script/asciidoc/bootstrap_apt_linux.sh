...
# Brew-based bootstrap script for setting up an Apt based system to build
# Clojure Cookbook
#
...
#
#  $ SOURCE_HIGHLIGHT_DIR=/foo/bar ./script/asciidoc/bootstrap_apt_linux.sh
#
...
test -f "$PLAIN_LANG_FILE" || (echo "********** Adding $PLAIN_LANG_FILE **********" && sudo cat <<END > "$PLAIN_LANG_FILE")
include "number.lang"
...
END
  ...
  sudo echo "$cleaned_contents" > "$LANG_MAP_FILE"
