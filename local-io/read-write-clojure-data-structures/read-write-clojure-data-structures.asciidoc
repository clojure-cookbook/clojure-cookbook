==== Reading and Writing Clojure Data Structures
...
Use +spit+ with +pr-str+ to serialize small amounts of data.
...
Use +read-string+ with +slurp+ to read small amounts of data.
...
IMPORTANT: When printing objects to disk via a stream, you must make
sure to close it in order to flush the stream, otherwise your data
might not appear on disk at all! The +with-open+ macro ensures that
the stream is flushed and closed after the body is executed.
