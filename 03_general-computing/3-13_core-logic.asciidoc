...
data.((("miniKanren Domain Specific Language")))
...
Prolog) from within Scheme. David Nolen created an implementation of miniKanren was
...
declarative style. By using +core.logic+, we are able to say _what_ we
are looking for in the graph without saying _how_ +core.logic+ should go
about finding it.

In general, all +core.logic+ queries begin with one of the library's
+run+ macros, with +clojure.core.logic/run+ returning a finite number of
solutions and +clojure.core.logic/run*+ returning _all_ of the
solutions.
...
program. A program is made up of _logic variables_ (created using
...
sides of the unification (the +clojure.core.logic/==+ relation) are
...
happens to only be +1+. The second example is the opposite, asking for any list
where +1+ is a member. The dot notation indicates an improper tail
with +_0+ in it. This means +1+ could be in a list by itself, or it
...
* The http://bit.ly/core-logic-wiki[+core.logic+ wiki]
* The http://minikanren.org/[miniKanren website]
