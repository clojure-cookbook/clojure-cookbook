...
in the United States that have "Clojure" anywhere in their job descriptions.(((databases, full-text search of)))(((Lucene library)))(((full-text search)))(((Clucy)))(((in-memory searches)))(((searching, full-text)))(((Java, Lucene library)))
...
(def stop-words
  (doto (CharArraySet. clucy.core/*version* 3 true)
    (.add "do")
    (.add "not")
    (.add "index")))
...
If no field is specified, there is an implicit field +_content+ that
