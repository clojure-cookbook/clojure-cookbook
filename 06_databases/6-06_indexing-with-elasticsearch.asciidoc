...
_Mappings_ define which fields in documents are indexed, which fields are tokenized, how fields are tokenized, which fields are analyzed, and so on. Each index in ElasticSearch may have
one or more _mapping types_. Mapping types can be thought of as
...
(although this analogy does not always stand).  Mapping types are the
...
* The official http://www.elasticsearch.org/guide/[Elasticsearch guide]
* The Elastisch http://clojureelasticsearch.info[home page]
