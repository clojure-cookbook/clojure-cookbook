...
<<sec_datomic_schema_solution>>. After doing this you should have a
connection, +conn+, on which to transact against and a schema on which
you can insert data against.
...
using the map syntax in the Solution, or any mix of the two.

One difference you'll note between the map and the expanded form is
the lack of a +:db/add+ statement for the +:db/id+ key itself. In the
...
// TODO: When would you not use :db.part/user?
...
evaluate once at _compile time_, which means that any code that
...
* See <<sec_datomic_schema>> for information on transacting schema.
