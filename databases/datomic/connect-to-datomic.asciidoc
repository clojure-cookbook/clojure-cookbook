...
* Free, for use with Datomic Free, subject to the 2-peer limit: +"datomic:free://[db-name]"+
* Dev, for use with Datomic Pro, subject to the licensed peer limit: +"datomic:dev://[db-name]"+

Free and Dev can also be configured to use alternate ports for storage:

+"datomic:free://localhost:4334/[db-name]?h2-port=[port]&amp;h2-web-port=[port]"+

====== External storage services:

External storage services are only available with Datomic Pro.

* Any of the following external storage services:
  * DynamoDB: +"datomic:ddb://[aws-region]/[dynamodb-table]/[db-name]?aws_access_key_id=[XXX]&aws_secret_key=[YYY]"+"
  * Riak: +"datomic:riak://host[:port]/bucket/dbname[?interface=http|protobuf]"+
