...
actually display the frame, we must use +show!+:
...
Having created the window, we can set its size, add content, and add scrollbars, as follows.

===== Adding Content
We can change properties of the frame using +config!+:
