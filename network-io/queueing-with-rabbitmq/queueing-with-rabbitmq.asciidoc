...
AMQP, and provides HTTP API client.
...
* Langohr http://reference.clojurerabbitmq.info[API reference] is also
  available.
* The http://www.rabbitmq.com/java-client.html[RabbitMQ Java Client],
  upon which Langohr is based; Clojure's Java interop is fairly easy
  to work with, and you may prefer to work with the Java client
  directly.
