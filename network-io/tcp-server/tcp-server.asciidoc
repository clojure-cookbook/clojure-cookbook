...
It then passes the socket to the +recieve+ function, which opens up a
reader on it and blocks until it recieves a full line of input,
terminated by a newline character (+\n+). When it recives one, it
...
demonstrate. Of course, an actual socket is not limted to strings, and
...
[source,clojure]
----
my-server> (def a (serve-persistent 8888 #(.toUpperCase %)))
#'my-server/a
;; server is running, will respond to multiple requests
my-server> (reset! a false)
false
