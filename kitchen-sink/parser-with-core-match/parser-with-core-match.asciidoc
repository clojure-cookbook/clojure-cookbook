...
Use _core.match_ to pattern match over the input and return the
...
* <<sec_core_match_interpreter>>, where the language used by this
  parser also appears in explaining how to write an interpreter
  for it. Readers that already recognize the language will probably
  find it completely unsurprising.
